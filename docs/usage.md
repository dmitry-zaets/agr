...
Click a sidebar step to open only its selected changes in a native diff, with up to three unchanged context lines on each side. Excerpt headers show original and modified file line ranges; the editor gutter numbers refer to the excerpt. Other items’ changes are excluded, even when they share a hunk. **AGR: Open Full Diff** (also in the sidebar toolbar and step context menu) opens the complete file comparison. Clicking the step again returns to its excerpts. Diffs use a reusable preview tab. Opening another step replaces that preview; pin the tab to keep a diff open. This follows VS Code’s editor preview setting. For steps with multiple files, a picker lets you choose which file to open. Notes start collapsed, with comment icons on the relevant lines. Click a comment icon to open the explanation. Check the step when finished; progress is stored in the guide. **Next Step** and **Previous Step** follow the guide's order.
