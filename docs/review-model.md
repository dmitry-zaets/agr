...
- Diffs use read-only excerpts of the selected changes, with up to three unchanged context lines bounded by other hunks. Headers label source line ranges; gutter numbers are excerpt coordinates. Metadata changes display their Git metadata rather than unrelated file contents. These excerpts are review views, not applicable patches. **AGR: Open Full Diff** shows the complete read-only file snapshots. Reopen a step to return to excerpts or see newer saved content. Highlighting marks the selected ranges on each side; notes appear as native comments. Pure deletions are annotated on the original side.
