...
4. Compute the merge-base of the fetched base and head. Use it and the PR head SHA in a revisions recipe. Put the PR URL, number, and chosen scope in the guide summary; group the review by behavior rather than GitHub's file order.

The extension itself is offline: it reads fetched Git objects. Authentication, metadata retrieval, and fetches happen through the agent's existing tools. If credentials or repository access are unavailable, report that blocker; do not silently review a different local diff. Reference: [GitHub CLI PR metadata](https://cli.github.com/manual/gh_pr_view).
